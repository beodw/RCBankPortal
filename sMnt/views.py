...
def sMntAddRecord(request):
	...
	try:
		...
		return JsonResponse({
			...
			'remaining_stock': int(init.final_serial_number) - int (init.initial_serial_number),
...
def sMntCancelStatments(request):
	# try:
        ...
        #remaining stock is zero after deletion and init serial num is == final serial num.
        remaining_stock = None
        if int(form['Cfinal_serial_number'][0]) == int (init.final_serial_number):
        	remaining_stock = 0
        else:
	...
	# except Exception as e:
	# 	print('\n\n\n',str(e),'\n\n\n')
	# 	return JsonResponse({'error':True,'message':'Could not cancel statement.'})
