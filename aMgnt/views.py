...
def asset_login(request):
	try:
		...
		if request.method == 'POST': #validate user
			...
			deactivated = None
			try :
				deactivated = Branch.objects.get(username=request.POST.get('username'),is_active=False)
			except:
				pass
			if deactivated:
				print('deactivated')
	...
	def get(self, request, *args, **kwargs):
		...
		try:
			if kwargs['type'] == 'Server':
				...
				hardware = System_unit.objects.get(id=kwargs['id'], is_received=True)
				holder = hardware.__dict__
		...
		#Handle invalid get request or errors in connecting to db.
			...
			return redirect('view_all_assets')
			...
			for branch in details['generate_for']:
				...
				for indexs, supply in enumerate( Supply.objects.filter(branch=branch) ):
					for index, hardware_component in enumerate( Hardware.objects.filter(supply=supply) ):
						...
						table = document.add_table(hardware_component.num_rows,5)
						...
						for key in hardware_component.__dict__:
							...
							if row != hardware_component.num_rows:
								if key != 'software_capable_hardware_ptr_id':#For some reason poping or usin del operation does not remove software_capable_hardware_ptr_id from self.__dict__
									key_name = None
									if key == 'is_replaced':
										key_name = 'Has been replaced'
									elif hardware_component.__dict__[key] == 'SystemUnit':
										hardware_component.__dict__[key] = 'Work Station'
									elif hardware_component.__dict__[key] is True:
										hardware_component.__dict__[key] = 'Yes'
...
class View_all_assets(View):
	...
	def aMgntViewAllAssetsTable(request, *args, **kwargs):
		...
		for index,word in enumerate(query):
		    ...
		    if 'workstation' in query[index].lower():
		    	query[index] = 'SystemUnit'
		...
		if request.user.username == 'IT SUPPORT' or request.user.username == 'Admin':
			if len(query) == 1:
				...
				query_set_holder = Hardware.objects.filter(query, is_replaced=False, is_received=True)
			...
			if len(query) is 1:
				...
				query_set_holder = Hardware.objects.filter(query,branch=request.user.username, is_replaced=False, is_received=True)
				hardware = query_set_holder[start:start+length]
			else:
				keywords = query
				for index, word in enumerate(keywords):
					if index == 0:
						...
						query &= Q(branch__icontains=word) | Q(hardware_type__icontains=word) | Q(brand_and_model__icontains=word) | Q(serial_number__icontains=word) | Q(condition__icontains=word) | Q(department__icontains=word) | Q(assigned_to__icontains=word) | Q(date_of_supply__icontains=word)
				query_set_holder = Hardware.objects.filter(query, is_received=True, is_replaced=False)
				hardware = query_set_holder[start:start+length]
