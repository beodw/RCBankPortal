...
# Clean form. Easier than implementing django clean() method in forms.py
# for each form.


def unpack(submitted_form):
    # Gen new dict because request.POST is immutable
    submitted_form = dict(submitted_form)
    ...
    if request.method == "GET":
        ...
        try:
            if authenticate(
                    ...
                    password=request.POST['adminPassword']) is None:
                return View_asset().get(
                    request,
                    id=request.POST['id'],
                ...
                if hardware.hardware_type == "SystemUnit":
                    ...
                    hardware = Software_capable_hardware.objects.get(
                        id=hardware.id)
                hardware.delete()
                return View_asset().get(
                    request,
...
class Manage_users(View):
    ...
    def post(self, request, *args, **kwargs):
        try:
            flag = False
            # check if admin raises error if not admin or admin_password field
            # missing from form.
            if authenticate(
                    username=request.user.username,
                    password=request.POST['admin_password']) is None:
                flag = True
            ...
            if request.POST['purpose'] == 'add_branch':
                ...
                if details['branch_password'] == details['verify_password'] and not bool(
                        Branch.objects.filter(name=details['branch_name'])):
                    ...
                    User.objects.create_user(
                        username=details['branch_name'],
                        password=details['branch_password'])
                    return self.get(request,
                                    **{'success': 'true',
                                       'error': 'false',
                                       'message': 'Branch ' + branch.name + ' successfully added.'})
                else:
                    # notify admin user that branch already exists or branch
                ...
                for name in details['branches_to_delete']:
                    ...
                    user.delete()
                    return self.get(request,
                                    **{'message': 'Branch ' + branch.name + ' has been deactivated.',
                                       'error': 'true',
                                       'success': 'false'})

            elif request.POST['purpose'] == 'reset_password':
                # Reset password for all branches chosen in table in manage
                # branches template.
                branches = ''
                if details.get('branches_to_reset') is None or len(
                        details['branches_to_reset']) < 1:  # if branches to select is removed fro form submission
                    raise Exception('Branches to reset cannot be empty')
                else:

                    for branch in details['branches_to_reset']:
                        # Check if branch already exists will throw error if
                        # not or if more than one in db.
                        branch = Branch.objects.get(name=branch)
                        if branch.is_active is True:
                            # because new_branch_password is list
                            password = make_password(
                                details['new_branch_password'][0])
                    ...
                    # if all branches to reset are deactivated branches.
                    if branches == '':
                        flag = True
                        raise Exception(
                            'The selected branch user(s) are deactivated. Their passwords cannot be reset.')
                    ...
                    return self.get(request, **{'success': 'true', 'error': 'false', 'message': 'Branch user(s): ' +
                                    branches + plural + ' ' + to_be_conjugation + ' been successfully reset.'})
...
class View_asset(View):
    ...
    def resolve_incident(self, details):
        ...
        incident.date_of_resolution = datetime.datetime.now().strftime(
            '%Y-%m-%d')  # defined above
        ...
        return redirect('/asset/viewAsset/' +
                        str(details['id']) +
                        '/' +
                        str(details['hardware_type']))
    ...
    def new_incident(self, details):
        ...
        return redirect('/asset/viewAsset/' + str(hardware.id) +
                        '/' + str(hardware.hardware_type))

    # Update hardware based on hardware type of post request.
    def update_hardware(self, details):
        details.pop('purpose')
        # Update details by making a model obj and saving with same id as
        # already existing obj.
        if details['hardware_type'] == 'Server':
        ...
        return redirect('/asset/viewAsset/' +
                        str(hardware.id) + '/' + hardware.hardware_type)
    ...
    def gen_replacement_history(self, hardware, list_of_replacements):
        ...
        if not bool(replacement):
            ...
            return self.gen_replacement_history(
                hardware=replacement.old_hardware,
    ...
    def replace_hardware(self, request, details):
        ...
        # Create new hardware
        # create new replacement using old hawrdware and new hardware details
        ...
        if System_unit_form_components(details).is_valid():
            ...
            old_hardware = Software_capable_hardware.objects.get(
                id=details['id'])
            old_hardware.is_replaced = True
        ...
        try:
            # New hardware must be saved first beacuse it may have a serial
            # number that already exists depending on user input. We need to
            # catch that.
            ...
            Replacement(
                ...
                date_of_replacement=datetime.datetime.now().strftime('%Y-%m-%d')).save()

            return redirect('/asset/viewAsset/' +
                            str(new_hardware.id) +
                            '/' +
                            str(new_hardware.hardware_type))
        except Exception as e:
            #print(e, '\n\n\n\n\n')
            error = 1
            ...
            return self.get(request,
                            **{'id': old_hardware.id,
                               'type': old_hardware.hardware_type,
                               'error': error,
                               'message': message})
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            if kwargs['type'] == 'Server':
                ...
                context['list_of_replacements'] = self.gen_replacement_history(
                    hardware, [])
                # hardware = hardware.__dict__
                holder.pop('hardware_ptr_id')
                # Update form components for Server and other software capable
                # hardware.
                ...
                context['software_capable_hardware_form_components'] = Software_capable_hardware_form_components(
                    initial=holder)
                context['update_system_unit_hardware_form_components'] = System_unit_form_components(
                    initial=holder)
                # Form components for replace hardware modal
                # becasuse Software_capable_hardware_form_components declared
                # above already has prepopulated fields.
                context['add_software_capable_hardware_components'] = Software_capable_hardware_form_components()
            ...
            # Raise exception if user is trying to access a replaced hardware
            # directly using url.
        ...
        # Handle invalid get request or errors in connecting to db.
            ...
            log(f'\n\n\n\n\n\n\n {e} \n To Handle invalid get request or errors fetching from db.')
            return render(request, 'generic404page.html', {'date': date})
    # Generic update form components
        context['update_generic_hardware_form'] = self.generic_update_form(
            holder)

        # Generic replace hardware form components.
        context['replace_hardware_generic_form'] = Replace_hardware_form(
            initial={'hardware_type': hardware.hardware_type, })
    ...
    def post(self, request, *args, **kwargs):
        try:  # Incase there is a error validating data or accessing db redirect to same hardware page.
            ...
            if details['purpose'] == 'update_hardware':
                ...
                hardware.save()
                return self.get(request,
                                **{'id': details['hardware_id'],
                                   'type': hardware.hardware_type,
                                   'message': 'Hardware successfully assigned to ' + details['staff_name'],
                                   'success': 'true',
                                   'error': 'false'})
        except Exception as e:
...
class Supplied_hardware(View):
    ...
    def gen_report(request, *args, **kwargs):
        try:
            ...
            document.add_heading(
                'ROKEL COMPUTER ASSETS REPORT \n' + str(
                    datetime.datetime.now().strftime('%d-%m-%Y')),
                0).alignment = WD_ALIGN_PARAGRAPH.CENTER
            flag = False
            details = dict(request.POST)
            del details['csrfmiddlewaretoken']
            details['generate_for'] = [
                branch for branch in details['generate_for'] if bool(
                    Supply.objects.filter(
                        branch=branch))]
            if (len(details['generate_for']) < 1):
                flag = True
                raise Exception(
                    'The selected branches do not have any hardware supplied.')
            ...
            for branch in details['generate_for']:
                ...
                hardware = []
                i = 0
                for indexs, supply in enumerate(
                        Supply.objects.filter(branch=branch)):
                    for index, hardware_component in enumerate(
                            Hardware.objects.filter(supply=supply)):
                        i = i + 1
                        if hardware_component.hardware_type == "SystemUnit":
                            document.add_heading(
                                "Hardware " + str(i) + " - WorkStation", level=2)
                        else:
                            document.add_heading(
                                "Hardware " + str(i) + " - " + hardware_component.hardware_type, level=2)
                        tabs = "\t\t\t\t"
                        if hardware_component.hardware_type == 'SystemUnit' or hardware_component.hardware_type == 'Laptop':
                            hardware_component = System_unit.objects.get(
                                id=hardware_component.id)
                        elif hardware_component.hardware_type == 'Server':
                            hardware_component = Software_capable_hardware.objects.get(
                                id=hardware_component.id)
                        del hardware_component.__dict__['supply_id']
                        hardware_component.clean()
                        table = document.add_table(
                            hardware_component.num_rows, 5)
                        del hardware_component.__dict__['num_rows']
                        row = 0
                        counter = 0
                        for key in hardware_component.__dict__:
                            if counter == 5:
                                counter = 0
                                row += 1
                            if row != hardware_component.num_rows:
                                # For some reason poping or usin del operation
                                # does not remove
                                # software_capable_hardware_ptr_id from
                                # self.__dict__
                                if key != 'software_capable_hardware_ptr_id':
                                    key_name = None
                                    if key == 'is_replaced':
                                        key_name = 'Has been replaced'
                                    elif hardware_component.__dict__[key] == 'SystemUnit':
                                        hardware_component.__dict__[
                                            key] = 'Work Station'
                                    elif hardware_component.__dict__[key] is True:
                                        hardware_component.__dict__[
                                            key] = 'Yes'
                                    elif key == 'system_unit_name':
                                        key_name = 'pC_name'
                                    if key_name is None:
                                        key_name = key
                                    holder = ''
                                    for word in key_name.split('_'):
                                        holder += word.capitalize() + " "
                                    table.cell(row, counter).add_paragraph(
                                        f'{holder}\n{hardware_component.__dict__[key]}').style = document.styles['Normal']
                            counter += 1
            f = io.BytesIO()
            document.save(f)
            f.seek(0)
            return FileResponse(
                f,
                as_attachment=True,
                filename='ROKEL COMPUTER ASSETS REPORT-' +
                datetime.datetime.now().strftime('%d-%m-%Y') +
                '.docx')
        except Exception as e:
            print('\n\n\n\n\n', e, '\n\n\n\n\n')
            if flag:
                return Supplied_hardware().get(
                    request, **{'message': str(e), 'error': 'true', 'success': 'false'})
            else:
    ...
    def get(self, request, *args, **kwargs):
        ...
        context['supply_hardware_form'].fields['department'] = forms.CharField(
            widget=forms.Select(attrs=gen_attrs()), label='Department or location')
        ...
        from django.contrib.auth import get_user_model
        branches = get_user_model().objects.filter(Q(is_superuser=0) & Q(
            is_staff=0) & ~Q(username='Admin') & ~Q(username='FHO'))
        # Gen dict of departments by branches e.g. { 'FHO':[ ['Risk','Risk'],
        # ['Legal', 'Legal'] ]  , 'FCHAR':[  ['Finance','Finance']... ] }
        import json
        ...
        context['Software_capable_hardware_form_components'] = Software_capable_hardware_form_components(
            initial={'operating_system': 'Windows Server 2012 R2'})
        context['System_unit_form_components'] = System_unit_form_components(
            initial={'operating_system': 'Windows 10 Pro'})
        ...
        # Gen branch choices
        branches = gen_choices(
            query_set=branches,
            attr='name',
            field_name='Branch')  # Choices is defined in forms.py
        ...
        if request.user.username != 'IT SUPPORT' and request.user.username != 'Admin':
            # system init is the status for hardware supplied before this
            # system was established.
            context['supplies'] = Supply.objects.filter(
                ~Q(status='System_Init'), branch=context['branch_name'])
        else:
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            # See if hardware with serial number already exists. Prevents
            # duplicate submissions and mistakes where s/n is the same.
            serial_numbers = []
            serial_numbers_str = ''
            ...
            for val in serial_numbers:
                if bool(Hardware.objects.filter(serial_number=val)):
                    return self.get(request,
                                    **{'error': 'true',
                                       'success': 'false',
                                       'message': 'A Hardware Component With Serial Number: ' + val + ' Already Exists'})

            # Make supply if it does not exist.
            # [0] are need to sccess details since unpack is not run on the incoming request.POST obj.
            details['date_of_supply'] = datetime.datetime.now().strftime(
                '%Y-%m-%d')

            try:  # Check if supply exists,Make new one if not.
                # Check if a supply with the same branch and date_of_supply
                # already exists. If not create a new supply.
                ...
                supply = Supply(
                    branch=details['branch'],
                    date_of_supply=details['date_of_supply'],
                    assigned_to=details['assigned_to'])  # Create new supply that hardware belongs to.
                supply.save()
                ...
                while i <= int(details['num_SystemUnit'][0]):
                    for key, val in details.items():
                        if 'SystemUnit' + str(i) in key:
                            holder[key.replace(
                                'SystemUnit' + str(i), '')] = details[key][0]
                    i += 1
                ...
                while i <= int(details['num_Other'][0]):
                    for key, val in details.items():
                        if 'Other' + str(i) in key:
                            holder[key.replace(
                                'Other' + str(i), '')] = details[key][0]
                        if '_type' in key:
                            if details['_typeOther' + str(i)][0] == 'Laptop':
                                holder['hardware_type'] = 'Laptop    '
                            else:
                                holder['hardware_type'] = details['_typeOther' +
                                                                  str(i)][0]
                    i += 1
                    holder.pop('_type')
            ...
            if str(e) == 'UNIQUE constraint failed: aMgnt_hardware.serial_number':
                return self.get(request, **{'message': self.error_hardware_holder.hardware_type + ' with serial number : ' +
                                self.error_hardware_holder.serial_number + ' already exists.', 'error': 'true'})
            return redirect('/asset/suppliedHardware')
...
class View_all_assets(View):
    ...
    def aMgntViewAllAssetsTable(request, *args, **kwargs):
        ...
        if request.user.username == 'IT SUPPORT' or request.user.username == 'Admin':
            if len(query) is 1:
                query = query[0]
                query = Q(
                    branch__icontains=request.POST['search[value]']) | Q(
                    hardware_type__icontains=query) | Q(
                    brand_and_model__icontains=query) | Q(
                    serial_number__icontains=query) | Q(
                    condition__icontains=query) | Q(
                        department__icontains=query) | Q(
                            assigned_to__icontains=query) | Q(
                                date_of_supply__icontains=query)
                query_set_holder = Hardware.objects.filter(
                    query, is_replaced=False, is_received=True)
                ...
                for index, word in enumerate(keywords):
                    if index == 0:
                        query = Q(
                            branch__icontains=word) | Q(
                            hardware_type__icontains=word) | Q(
                            brand_and_model__icontains=word) | Q(
                            serial_number__icontains=word) | Q(
                            condition__icontains=word) | Q(
                            department__icontains=word) | Q(
                            assigned_to__icontains=word) | Q(
                                date_of_supply__icontains=word)
                    else:
                        query &= Q(
                            branch__icontains=word) | Q(
                            hardware_type__icontains=word) | Q(
                ...
                query = Q(
                    ...
                    department__icontains=query) | Q(
                        assigned_to__icontains=query) | Q(
                            date_of_supply__icontains=query)
                query_set_holder = Hardware.objects.filter(
                    query, branch=request.user.username, is_replaced=False, is_received=True)
                hardware = query_set_holder[start:start + length]
            else:
                keywords = query
                for index, word in enumerate(keywords):
                    if index == 0:
                        ...
                        query &= Q(
                            ...
                            assigned_to__icontains=word) | Q(
                                date_of_supply__icontains=word)
                query_set_holder = Hardware.objects.filter(
                    query, is_received=True, is_replaced=False)
                hardware = query_set_holder[start:start + length]
        for hardware in hardware:
            ...
            holder = {
                'DT_RowId': f'{hardware.id}/{hardware.hardware_type}/',
                'Date of Supply': hardware.date_of_supply,
                'Department': hardware.department,
                'Assigned Role': hardware.assigned_to,
                'Serial Number': hardware.serial_number,
                'Brand and Model': hardware.brand_and_model,
                'Type': hardware.hardware_type,
        ...
        return JsonResponse({'draw': int(request.POST['draw']), 'recordsTotal': Hardware.objects.all(
        ).count(), 'recordsFiltered': query_set_holder.count(), 'data': result})

    def get(self, request, *args, **kwargs):

        context = {'date': date, 'branch_name': request.user.username}

        # Limit to hardware to that which belong to a branch.
        if request.user.username != 'IT SUPPORT' and request.user.username != 'Admin':
            context['assets'] = [
                Hardware.objects.filter(
                    supply=supply,
                    is_replaced=False,
                    is_received=True) for supply in Supply.objects.filter(
            ...
            # 	if count is 100:
            # 		break;
        # Gen message for user depending on previous input from post section of
        # this view.
        context['success'] = 0
        context['error'] = 0
        if bool(kwargs):
        ...
        # Temporary function to add hardware for easy integration of syste
        ...
        context['supply_details_form'].fields['date_of_supply'].widget.attrs['max'] = datetime.datetime.now(
        ).strftime('%Y-%m-%d')
        ...
        context['supply_details_form'].fields['department'] = forms.CharField(label='Location', widget=forms.Select(
            attrs=gen_attrs({'class': 'form-control', 'required': '', 'placeholder': 'e.g. Finance Department'})))
        context['supply_details_form'].fields['department'].widget.choices = [
            (branch.name,
             branch.name) for branch in Branch.objects.filter(
                ~Q(
                    name="IT SUPPORT"),
                ~Q(
                    name="Admin"))] + [
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
                ...
                for key, val in request.POST.items():
                    if 'serial_number' == key[:13]:
                        if val == "NA":
                            ...
                            raise Exception(
                                'Serial number : ' + str(val) + ' already exists.')
                            break
            except Exception as e:
                return redirect(
                    'view_all_assets_success',
                    message=str(e),
                    success='False',
                    error='True')
            try:
                supply = Supply.objects.get(
                    assigned_to='System Init',
                    date_of_supply=datetime.datetime.now().strftime('%Y-%m-%d'),
            ...
            return redirect(
                'view_all_assets_success',
                message='Hardware component(s) added successfully',
                error='False',
                success='True')
        except Exception as e:
            print('\n\n\n', e, '\n\n\n\n')
        ...
        # return self.get(request, kwargs={'success':True, 'message':'New
        # Department Succesfully Added'})

        # 		else:

        # return self.get(request, kwargs={'error':True, 'message':'Department
        # Already Exists'})
...
class Supply_details(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for hardware in supplied_hardware:
            if hardware.hardware_type == 'Server':
                list_of_hardware.append(
                    Software_capable_hardware.objects.get(
                        id=hardware.id).__dict__)
            elif hardware.hardware_type == 'SystemUnit' or hardware.hardware_type == 'Laptop':
                list_of_hardware.append(
                    System_unit.objects.get(
                        id=hardware.id).__dict__)
            else:
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            # Section for validating each hardware part of supply in a
            # hardware = Hardware.objects.get(id=details['id'])
            # hardware.is_received = True
            # hardware.save()
            ...
            if details['purpose'] == 'confirm_hardware':
                user = authenticate(
                    username=request.user.username,
                    password=details['branch_password'])  # check if branch password is correct
                if user is not None:
                    supply = Supply.objects.get(id=int(details['sup_id']))
                ...
                return self.get(request,
                                **{'id': details['sup_id'],
                                   'success': 'true',
                                   'error': 'false',
                                   'message': 'This supply has been sent to IT SUPPORT for revision. This page will be updated after they make changes.'})
                ...
                return self.get(request,
                                **{'id': details['sup_id'],
                                   'success': 'true',
                                   'error': 'false',
                                   'message': 'The selected hardware has been updated.'})
            elif details['purpose'] == 'resend_supply':
                supply = Supply.objects.get(id=details['supply_id'])
                supply.status = 'Pending'
                supply.save()
                return self.get(request,
                                **{'id': details['supply_id'],
                                   'success': 'true',
                                   'error': 'false',
                                   'message': 'This supply has been resent to branch ' + supply.branch})
                ...
                supply = Supply.objects.get(id=int(details['sup_id']))
                supply.delete()
                return Supplied_hardware().get(request,
                                               **{'success': 'true',
                                                  'error': 'false',
                                                  'message': 'Supply for ' + supply.branch + ' on ' + supply.date_of_supply.strftime('%B %e, %Y') + ' successfully deleted.'})
            return redirect('/asset/suppliedHardware/')
        except Exception as e:
            # print("\n\n\n\n", e, '\n\n\n\n\n')
            return redirect('/asset/suppliedHardware/')
        ...
        if details['purpose'] == 'print_supply':
            ...
            for hardware in hardwares:
                if hardware.hardware_type == 'Server':
                    hardware = Software_capable_hardware.objects.get(
                        id=hardware.id)
                    hardware_types.update(
                        {'server': hardware_types['server'] + 1})
                elif hardware.hardware_type == 'SystemUnit':
                    hardware = System_unit.objects.get(id=hardware.id)
                    hardware_types.update(
                        {'desktop': hardware_types['desktop'] + 1})
                else:
                    if hardware_types.get(
                            hardware.hardware_type.lower()) is None:
                        hardware_types.update(
                            {hardware.hardware_type.lower(): 1})
                    else:
                        hardware_types.update({hardware.hardware_type.lower(
                        ): hardware_types[hardware.hardware_type.lower()] + 1})

            is_plural = 0
            for key, val in hardware_types.items():
                is_plural = is_plural + val
            # if more than one hardware in supply check if all are of the same
            # type.
            all_the_same = False
            for key, val in hardware_types.items():
                if val > 0:
            ...
            document.add_picture(
                './aMgnt/static/aMgnt/assets/logo/RokelLogoHeader.jpg',
                width=Inches(1.0),
                height=Inches(1.0))
            ...
            document.add_paragraph(
                "Your Ref: " +
                supply.branch +
                "\t\t\t\t\t\t Our Ref: AL/[Insert Staff Initials Here]")
            document.add_paragraph()
            document.add_paragraph(
                'Date: ' + str(supply.date_of_supply.strftime('%d-%B-%Y')).replace('-', ' '))
            ...
            for key, val in hardware_types.items():
                ...
                # make word plural e.g. server => servers, desktop => desktops,
                # mouse => mice etc...
            ...
            document.add_paragraph(
                ...
                " " +
                to_be_conjugation +
                " been supplied to your branch.")
            document.add_paragraph()
            document.add_paragraph("Description \t \t \t \t \t \t  Serial No")
            for hardware in hardwares:
                if hardware.hardware_type != "SystemUnit":
                    document.add_paragraph(
                        hardware.hardware_type.upper() +
                        "\t \t \t \t \t \t  CN=" +
                    ...
                    document.add_paragraph(
                        "KEYBOARD\t \t \t \t \t \t  CN=" +
                        hardware.keyboard_serial_number)
                    document.add_paragraph(
                        "MOUSE\t \t \t \t \t \t  CN=" +
                        hardware.mouse_serial_number)
            ...
            return FileResponse(f, as_attachment=True, filename="Supply For " +
                                supply.branch + ' ' + str(supply.date_of_supply) + ".docx")
        return redirect('/asset/suppliedHardware/')
        # except Exception as e:
        # 	print('\n\n\n\n\n\n', e, '\n\n\n\n')
        # 	return redirect('/asset/suppliedHardware/')
...
class aMgntGeneralIncidents(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request,
                      self.template,
                      {'date': date,
                       'generalIncidents': bool(GeneralIncident.objects.all()),
                       'branches': Branch.objects.filter(~Q(name="IT SUPPORT")),
                       'message': message,
                       'success': success})
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            return JsonResponse({'success': True,
                                 ...
                                 'newIncidentId': new_incident.id})
        except Exception as e:
            log("\n\n\n" + str(e) + "\n\n\n")
            return JsonResponse(
                {'success': False, 'message': 'Could not add incident.'})
        ...
        try:
            if request.method == 'POST':
                ...
                if incident.resolved is not 1:  # not resolved.
                    ...
                    return JsonResponse(
                        {'success': False, 'message': 'Incident has already been resolved.'})
                return JsonResponse(
                    {'success': True, 'message': 'Incident successfully updated.'})
            else:
                response = HttpResponse()
            ...
            log('\n\n\n' + str(e) + '\n\n\n')
            return JsonResponse(
                {'success': False, 'message': 'Error updating incident.'})

    def get_incidents(request):
        try:
            query = request.POST['search[value]']
            query = Q(
                branch__contains=query) | Q(
                reportingStaffName__icontains=query)
            if request.user.username == 'Admin' or request.user.username == 'IT SUPPORT':
                general_incidents = GeneralIncident.objects.filter(query).order_by(
                    '-date')[int(request.POST['start']):int(request.POST['start']) + int(request.POST['length'])]
            else:
                general_incidents = GeneralIncident.objects.filter(query, branch=request.user.username).order_by(
                    '-date')[int(request.POST['start']):int(request.POST['start']) + int(request.POST['length'])]
            if bool(general_incidents):
                if request.user.username == 'Admin' or request.user.username == 'IT SUPPORT':
                    rows = [{'DT_RowId': incident.id,
                             'Branch': incident.branch,
                             'Date': incident.date.strftime('%A %d. %B %Y'),
                             'Time': incident.date.strftime('%H:%m:%S'),
                             'Description': incident.description,
                             'Reporting Staff': incident.reportingStaffName,
                             'Severity': incident.get_severity(),
                             'Status': incident.get_status()}for incident in general_incidents]
                else:
                    rows = [{'DT_RowId': incident.id,
                             'Date': incident.date.strftime('%A %d. %B %Y'),
                             'Time': incident.date.strftime('%H:%m:%S'),
                ...
                return JsonResponse(
                    {
                        'draw': int(
                            request.POST['draw']),
                        'recordsTotal': general_incidents.count(),
                        'recordsFiltered': GeneralIncident.objects.all().count(),
                        'data': rows})
            else:
                return JsonResponse(
            ...
            log('\n\n\n' + str(e) + '\n\n\n')
            return JsonResponse(
                {'success': False, 'message': 'Error retrieving incidents.', 'data': []})
...
class aMgntGeneralIncident(View):
    def get(self, request, *args, **kwargs):
        try:
            ...
            if bool(kwargs.get('message')):
                ...
                message = ''
                success = '-1'
            incident = GeneralIncident.objects.get(
                id=int(kwargs['incidentId']))
            return render(
                request, 'aMgntViewIncident.html', {
                    'date': date, 'incident': incident, 'success': success, 'message': message})
        except Exception as e:
            log('\n\n\n' + str(e) + '\n\n\n')
            return aMgntGeneralIncidents().get(
                request, message="Could not load incident details", success='0')
    ...
    def delete_incident(request):
        try:
            ...
            if user is None:
                return aMgntGeneralIncident().get(
                    request,
            ...
            return aMgntGeneralIncidents().get(
                request,
                success='1',
                message='Incident for ' +
                incident.branch +
                ' has been deleted.')
        except Exception as e:
            log('\n\n\n' + str(e) + '\n\n\n')
            return aMgntGeneralIncident().get(
                request, success="0", message="Could not delete incident.")
...
class aMgntQRScanner(View):
    ...
    def post(self, request, *args, **kwargs):
        try:
            hardware = Hardware.objects.get(id=request.POST['id'])
            return JsonResponse({'success': True,
                                 'hardwareDetails': {'supplyId': hardware.supply.id,
                                                     'status': hardware.supply.status,
                                                     'id': hardware.id,
                                                     'Branch': hardware.supply.branch,
                                                     'Department': hardware.department,
